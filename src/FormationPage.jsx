...
import { 
  ...
  ChevronRight, Zap, BookOpen, Rocket 
} from 'lucide-react';

const FormationPage = ({ onContactClick }) => {
  return (
    <div className="animate-in fade-in duration-700 text-left bg-white">
      
      {/* --- HERO ACADÉMIE --- */}
      <section className="relative pt-24 min-h-[70vh] flex items-center overflow-hidden bg-emerald-900 text-white">
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="max-w-4xl">
            <h2 className="text-amber-400 font-black text-sm uppercase tracking-[0.5em] mb-6">Académie d'Excellence</h2>
            <h1 className="text-6xl md:text-[100px] font-black uppercase leading-none mb-12">
              L'Avenir par le <br/>
              <span className="text-amber-400 italic font-serif normal-case tracking-normal">Savoir-Faire.</span>
            </h1>
      ...
      {/* --- GRILLE DES PROGRAMMES (NETTOYÉE ET PASSÉE EN 4 COLONNES) --- */}
      ...
      <section className="py-32 px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <h3 className="text-center text-sm font-black text-amber-500 uppercase tracking-[0.5em] mb-20">La Méthode MARISA</h3>
...
// --- SOUS-COMPOSANT CARTE (MIS À JOUR SANS LISTE) ---
const ProgramCard = ({ img, title, icon }) => (
  <div className="group relative h-[450px] rounded-[2.5rem] overflow-hidden shadow-xl border border-slate-100 text-left">
    <img 
      src={img} 
      ...
      alt={title} 
    />
    {/* Overlay dégradé plus intense en bas pour la lisibilité */}
    <div className="absolute inset-0 bg-gradient-to-t from-emerald-950 via-emerald-950/20 to-transparent opacity-90" />
    
    {/* Icône flottante */}
    <div className="absolute top-6 right-6 bg-white/20 backdrop-blur-xl p-4 rounded-2xl text-white transform group-hover:rotate-6 transition-all border border-white/10">
      {icon}
    </div>

    {/* Titre en bas */}
    <div className="absolute bottom-0 p-8 w-full">
      <h4 className="text-2xl font-black text-white uppercase tracking-tighter leading-tight group-hover:text-amber-400 transition-colors">
        {title}
      </h4>
    </div>
  </div>
